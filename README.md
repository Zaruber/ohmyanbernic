...
Завалялся дома Anbernic RG405V — ретро-консоль на Android. А тут ботов для Telegram некуда повесить: за VDS платить — с жиру беситься, а дома без дела лежит вполне рабочее устройство с ARM-процессором, Wi-Fi и Linux-окружением через Termux. Ну и чего бы нет.

Поставил Termux, поднял SSH, подключился с мака. Собрал в Antigravity веб-морду на чистом Python (без зависимостей), закинул на консоль через `scp`, запустил. Дашборд даёт мониторинг системы с графиками истории, файловый менеджер для навигации по устройству, а также менеджер процессов — можно смотреть что запущено, убивать ненужное, и запускать новые Python-скрипты прямо из браузера, указав путь к файлу. Всё работает по локалке, без облаков и подписок.
